...
(function ($) {
    ...
    var settings = {}, roots = {}, caches = {},
        ...
        // zTree默认配置对象
        _setting = {
            treeId: "", //zTree的唯一标识，一般是id属性值
            treeObj: null, //zTree 容器的 jQuery 对象，主要功能：便于操作
            view: {
                // addDiyDom用于在节点上固定显示用户自定义控件
                // addDiyDom值是一个函数，该函数接收两个参数：
                // treeId：对应 zTree 的 treeId
                // treeNode：需要显示自定义控件的节点 JSON 数据对象
                addDiyDom: null,
                // autoCancelSelected：点击节点时，按下 Ctrl 或 Cmd 键是否允许取消选择操作
                autoCancelSelected: true,
                // dblClickExpand：双击节点时，是否自动展开父节点的标识
                // dblClickExpand还可以接受一个函数，用来控制不同级别节点的不同功能
                dblClickExpand: true,
                // expandSpeed：zTree 节点展开、折叠时的动画速度
                expandSpeed: "fast",
                // fontCss：个性化文字样式，只针对 zTree 在节点上显示的<A>对象
                // 值可以是表示css样式的json对象，或者是返回json对象的函数
                // 当值是函数时，接受两个参数：treeId，treeNodeJSON（需要设置自定义样式的节点 JSON 数据对象）
                fontCss: {},
            ...
            data: {
                ...
                simpleData: {
                    // 确定 zTree 初始化时的节点数据、异步加载时的节点数据、或 addNodes 方法中输入的 newNodes 数据是否采用简单数据模式 (Array)，即不需要用户再把数据库中取出的 List 强行转换为复杂的 JSON 嵌套格式
                    enable: false,
                    // 节点数据中保存唯一标识的属性名称
                    idKey: "id",
                ...
                keep: {
                    ...
                    parent: false,
                    // zTree 的节点叶子节点属性锁
                    // 如果设置为true，则所有 isParent = false 的节点，都无法添加子节点
                    leaf: false
                }
            },
            async: {
                ...
                dataType: "text",
                // 设置固定的异步加载 url 字符串
                // 可以是一个返回地址的函数，该函数接受两个参数：treeId，treeNode（需要异步加载子节点的的父节点 JSON 数据对象。针对根进行异步加载时，treeNode = null）
                url: "",
                // 异步加载时需要自动提交父节点属性的参数，值是一个数组或者返回数组的函数
                autoParam: [],
                // Ajax 请求提交的静态参数键值对
                otherParam: [],
                // 用于对 Ajax 返回数据进行预处理的函数
                // 接受三个参数：treeIdS，parentNode（进行异步加载的父节点 JSON 数据对象，对根进行异步加载时，parentNode = null），responseData（异步加载获取到的数据转换后的 Array(JSON) / JSON / String 数据对象）
                // 返回值：zTree 支持的JSON 数据结构即可
                dataFilter: null
            },
            callback: {
                beforeAsync: null,
    ...
    $.fn.zTree = {
        ...
        /**
         * init方法时ztree的初始化方法
         * @param obj 用于展现zTree的DOM容器
         * @param zSetting zTree的配置数据
         * @param zNodes zTree的节点数据
